...
model = TD3("MlpPolicy", env, verbose=1,action_noise=action_noise,learning_starts=int(1000),gamma=0.98, gradient_steps=-1,
            train_freq=((100, "step")),learning_rate=1e-3,buffer_size=int(1e5), policy_delay=2,batch_size=256,policy_kwargs=policy_kwargs)

log_Path = os.path.join(os.path.dirname(__file__), "log_trains/model_1_3")
new_logger = configure(log_Path, ["stdout", "csv", "tensorboard"])
model.set_logger(new_logger)

model.learn(total_timesteps=int(5e5),log_interval=1,tb_log_name="1_3_log")

model_Path = os.path.join(os.path.dirname(__file__), "log_models/model_1_3")
