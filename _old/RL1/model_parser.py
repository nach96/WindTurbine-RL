...
class ModelParser():
    ...
    def setup_env(self,input_file_rel_path):
        self.env = FastGym_1(inputFileName=self.h.get_file_path(input_file_rel_path), **self.FAST_params, enable_myLog=True, myLogName="td3_1")
    ...
    def setup_logger(self):
        log_Path = self.h.get_file_path("log_trains/model_1_"+self.model_ID)
...
class HelperOFRL():
    ...
    def log_and_exit(self, model, env, id):
        model_Path = self.get_file_path("log_models/model_1_"+id)
        model.save(self.name_date(model_Path,""))
        beepy.beep(sound=5)

        # Plot Logged variables during training
        log_Path = self.get_file_path("log_trains/model_1_"+id)
